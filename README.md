...
Inferno Stats' existing saved-wave URLs can be pasted into the website. [PR #20](https://github.com/InfernoStats/InfernoStats/pull/20) adds this site to its **Wave Tool** selector, with one **Current LoS** button and existing wave-row spawn links using its original query format plus captured player, pillar and NPC-index data. Sharing on the website generates compact IL2 codes; see [the integration notes](docs/INFERNO_STATS.md).
